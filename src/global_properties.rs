    ...
    assert!(properties
        ...
        .insert(
            ...
            Arc::new(PropertyAccessors {
                setter: Box::new(
                    |context: &mut Context, value| -> Result<(), IxaError> {
                ...
                getter: Box::new(|context: &Context| -> Result<Option<String>, IxaError> {
                    let value = context.get_global_property_value::<T>();
                    match value {
                        Some(val) => Ok(Some(serde_json::to_string(val).map_err(|e| IxaError::from(e))?)),
                        None => Ok(None),
                    }
                }),
            })
...
macro_rules! define_global_property {
    ($global_property:ty, $validate: expr) => {
        ...
        impl New for $global_property {
            const new: &'static dyn Fn() -> Self = &<$global_property>::default;
        }
...
mod test {
    ...
    define_global_property!(DiseaseParams);
    ...
    fn set_parameters() {
        ...
        let params_json = context
            .load_parameters_from_json::<DiseaseParams>(&file_path)
            .unwrap();

        context
            .set_global_property_value::<DiseaseParams>(params_json)
            .unwrap();
    ...
    fn read_global_properties() {
        ...
        let p2 = context.get_global_property_value::<Property2Type>().unwrap();
    ...
    fn validate_property_set_success() {
        let mut context = Context::new();
        context
            .set_global_property_value::<Property3Type>(Property3Type { field_int: 0 })
            .unwrap();
    }
    ...
    fn get_serialized_value_by_string() {
        let mut context = Context::new();
        context
            .set_global_property_value::<DiseaseParams>(
                DiseaseParams {
                    days: 10,
                    diseases: 2,
                },
